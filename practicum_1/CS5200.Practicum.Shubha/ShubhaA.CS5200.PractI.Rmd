...
email: adithyaaravi10@gmail.com
date: "`r Sys.Date()`"
output:
  html_document:
    df_print: paged
...
bds.raw <- read.csv(file = "BirdStrikesData-V4-SuF24 copy.csv",
                    header = TRUE,
...
cleanUpData <- function(dataFrame) {
  ...
  sentinelValues <- list(
    ...
    # Setting it to minimum value of date in mysql.
    # referredFrom: https://dev.mysql.com/doc/refman/8.4/en/datetime.html
    flight_date = "01/01/1000 0:00",
...
topStatesWithStrikes <- function(dbCon) {
  query <- sprintf(
    "SELECT a.airportState, COUNT(*) as incidents
  FROM %s a
...
```{r updateWildLifeStrike, echo=FALSE,warning=FALSE, eval= FALSE}
