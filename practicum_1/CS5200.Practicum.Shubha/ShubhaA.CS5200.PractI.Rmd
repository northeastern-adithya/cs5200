...
bds.raw <- read.csv(file = "BirdStrikesData-V4-SuF24 copy.csv", header = TRUE, stringsAsFactors = FALSE)
```

```{r insertDataIntoTables, echo=FALSE,warning=FALSE, eval= TRUE}
...
query <- sprintf("SELECT a.airportState, COUNT(*) as incidents
FROM %s a
JOIN %s f ON a.aid = f.originAirport
JOIN %s ws ON f.fid = ws.fid
GROUP BY a.airportState
ORDER BY incidents DESC
LIMIT 5",airportsTableName,flightsTableName,widlifeStrikesTableName)
...
airportsBelowAverageStrikes <- function(dbCon){
  query <- sprintf("WITH strikeCount AS (
      SELECT 
        a.aid,
        a.airportName,
    ...
    LIMIT 5", widlifeStrikesTableName, flightsTableName, airportsTableName)
...
strikesByWildlifeSize <- function(dbCon){
  ...
  ", widlifeStrikesTableName, flightsTableName, wildlifeSizeTableName)
  return (dbGetQuery(dbCon, query))
}
...
kable(resultTable,
  format = "html", 
        col.names = c("Year", "Small", "Medium", "Large", "Total")) %>%
  kable_styling(
    # striped is used to get color contrast between rows.
    bootstrap_options = c("striped"),
    full_width = FALSE,
...
visualiseTrendByYear <- function(dataFrame){
  ...
  plot(dataFrame$strikeYear, dataFrame$total,
       type = "o",  
       col = "darkblue",
       pch = 19,    
       lwd = 2,   
       ...
       ylim = c(0, max(dataFrame$total) * 1.2)) 
  

   grid(lty = 2, col = "gray80")
  text(dataFrame$strikeYear, dataFrame$total + 10,
